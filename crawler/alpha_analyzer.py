...
class AlphaAnalyzer:
    ...
    def analyze_stock(self, symbol):
        ...
        try:
            ...
            fund_report = f"기본적 분석상 REAL 데이터 확인 결과, 수익률(ROA) {round(real_roic, 2)}% 및 영업마진 {round(real_margin, 2)}%를 기록 중입니다. 이는 시장 컨센서스를 {'상회하는' if real_margin > 15 else '준수하는'} 수준이며, 시가총액 {unit}{round(info.get('marketCap', 0)/1e9, 1)}B 규모의 펀더멘털 건전성을 입증합니다."
